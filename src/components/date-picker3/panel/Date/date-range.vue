        ...
        <div :class="panelBodyClasses">
            <div :class="[prefixCls + '-content', prefixCls + '-content-left']" v-show="!isTime">
                ...
                <component
                    ...
                    :panelPosition="splitPanels ? 'left' : undefined"
            ...
            <div :class="[prefixCls + '-content', prefixCls + '-content-right']" v-show="!isTime">
                ...
                <component
                    ...
                    ref="rightYearTable"
                    v-if="currentView !== 'time'"
                    :panelPosition="splitPanels ? 'right' : undefined"
...
<script>  
    ...
    import splitPanelMixin from './split-panel-mixin';
    ...
    export default {
        ...
        mixins: [ Mixin, Locale, DateMixin, splitPanelMixin ],
        ...
        data(){
            ...
            const splitPanels = this.splitPanels;
            ...
            return {
                ...
                rightPanelDate: splitPanels ? new Date(leftPanelDate.getFullYear(), leftPanelDate.getMonth(), 1) : new Date(leftPanelDate.getFullYear(), leftPanelDate.getMonth() + 1, 1)
            };
        },
        computed: {
            classes(){
            ...
            panelBodyClasses(){
                // when the panel is showing week numbers and dates, it takes up a larger part; whereas it takes up a normal part;
                let isLeftShowingDates = this.leftPickerTable === 'date-table' && this.currentView === 'date' // panel showing dates only
            ...
            panelPickerHandlers(){
                return {
                    left: this.preSelecting.left ? 
                        ...
                        (this.splitPanels ? this.handleRangeDatePick.bind(this, 'left', dateSorter) : this.handleRangePick),
                    right: this.preSelecting.right ? 
                        this.handlePreSelection.bind(this, 'right') : 
                        (this.splitPanels ? this.handleRangeDatePick.bind(this, 'right', dateSorter) : this.handleRangePick),
                };
            }
        },
        watch: {
            value(newVal) {
                ...
                this.dates = this.dateRangeValidate([minDate, maxDate], dateSorter);
            ...
            currentView(currentView){
                ...
                if (currentView === 'date' && isSameYear && leftMonth === rightMonth){
                    ...
                    !this.splitPanels && this.changePanelDate('right', 'Month', 1);
                }
        ...
        methods: {
            ...
            setPanelDates(leftPanelDate){
                ...
                if (this.splitPanels) {
                    rightPanelDate = this.dates[1] ? new Date(this.dates[1]) : new Date(leftPanelDate.getFullYear(), leftPanelDate.getMonth(), 1);
                } else {
            ...
            handlePreSelection(panel, value){ // 处理年月等选择
            ...
            handleRangePick (val, type) { // 处理时间点击
                if (this.rangeState.selecting || this.currentView === 'time'){
                    if (this.currentView === 'time'){
                        this.dates = val;
                    } else {
                        // const [minDate, maxDate] = [this.rangeState.from, val].sort(dateSorter);
                        const [minDate, maxDate] = this.dateRangeValidate([this.rangeState.from, val], dateSorter);
            ...
            handleChangeRange (val) {
                ...
                //
                if(this.splitPanels) return;
                this.rangeState.to = val;
                //
            },
            dateRangeValidate (arr, dateSorter) {
                let res = [];

                if(this.splitPanels){
                    let sDate = arr[0];
                    let eDate = arr[1];
                    let currentPanel = this.currentPanel || 'left';

                    if(sDate != null && eDate != null && sDate.valueOf() > eDate.valueOf()){
                        // currentPanel === 'left' ? eDate = null : sDate = null;
                        currentPanel === 'left' ? eDate = new Date(sDate) : sDate = new Date(eDate);
                    }

                    res = [sDate, eDate];
                } else {
                    res = arr.sort(dateSorter);
                }
